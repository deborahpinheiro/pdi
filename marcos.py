...
def parse_lista_precos(pdf_path: str) -> pd.DataFrame:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        # === pega texto bruto da LISTA ===
        header_text = pdf.pages[0].extract_text() or ""
        m = re.search(r"LISTA:\s*(.+?)(?:\s{2,}|VALIDADE|BATIDA|PÁG|$)",
                      header_text,
                      flags=re.IGNORECASE)
        raw_lista = m.group(1).strip() if m else ""

        # === define lista (INSUMOS / PET) com base no texto bruto ===
        lista = None
        ...
        for page_idx, page in enumerate(pdf.pages, start=1):
            ...
            for table in tables:
                ...
                for row in table:
                    ...
                    # linha de família
                    if (
                        c0 and not c1 and not c2 and not c3
                        and not any(k in joined for k in ["CÓD", "COD", "PRODUTO", "R$/TON", "R$/SC"])
                    ):
...
if __name__ == "__main__":
    pdf_path = r"C:/Users/efhqc/OneDrive - Bayer/Downloads/[101025] - INS VOTORANTIM 15.pdf"
    df = parse_lista_precos(pdf_path)

    print(df.head(20))
    print("Total de linhas:", len(df))

    df.to_csv(
        r"C:/Users/efhqc/OneDrive - Bayer/Downloads/[101025] - INS VOTORANTIM 15.csv",
        index=False,
        encoding="utf-8-sig"
    )
